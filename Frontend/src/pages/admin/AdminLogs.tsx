...
const AdminLogs = () => {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const data = await getLogs({ ...filters, page, limit: 20 });
      setLogs(data.logs);
  ...
  const handleExport = async (format: string) => {
    try {
      const response = await exportLogs({ ...filters, format });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `admin_logs.${format === 'excel' ? 'xlsx' : format}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Activity Log Book</h2>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="flex items-end">
            <button
              onClick={() => {
                setFilters({ startDate: '', endDate: '', adminId: '', action: '', entity: '' });
                setPage(1);
              }}
